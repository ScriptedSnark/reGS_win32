	...
	: BaseClass(parent, nullptr)
{
	...
	vgui2::localize()->ConvertUnicodeToANSI(
		vgui2::localize()->Find("#GameUI_Software"),
		m_pszRenderNames[0],
	...
	vgui2::localize()->ConvertUnicodeToANSI(
		...
		m_pszRenderNames[1],
		ARRAYSIZE(m_pszRenderNames[1]));

	vgui2::localize()->ConvertUnicodeToANSI(
		vgui2::localize()->Find("#GameUI_D3D"),
		m_pszRenderNames[2],
		ARRAYSIZE(m_pszRenderNames[2]));


	m_pRenderer = new vgui2::ComboBox(this, "Renderer", 3, false);
	for (int i = 0; i < 3; i++)
	{
		m_pRenderer->AddItem(m_pszRenderNames[i], NULL);
	}

	m_pRenderer->SetItemEnabled(0, true);
