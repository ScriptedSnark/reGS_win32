	...
	: BaseClass(parent, "LoadingDialog", true)
{
	...
	if (TheCareerGame && TheCareerGame->IsPlayingMatch())
	{
		...
		Color normalColor = pScheme->GetColor("ControlText", Color(255, 255, 255, 255));
		Color friendlyFireColor = pScheme->GetColor("BrightControlText", Color(255, 255, 255, 255));
...
void CLoadingDialog::DisplayGenericError(const char* failureReason, const char* extendedReason)
{
	...
	if (*extendedReason == '#' && ((pszLocalized = vgui2::localize()->Find(extendedReason)) != nullptr))
	{
	...
	if (*failureReason == '#' && ((pszLocalized = vgui2::localize()->Find(failureReason)) != nullptr))
	{
